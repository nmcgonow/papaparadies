...
type Phase = "loading" | "walking" | "objects" | "bubble" | "zoom" | "door" | "paradise" | "done";
  ...
  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ opacity: 0.07 }}
      ...
      {Array.from({ length: 24 }, (_, i) => {
        ...
        return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#0D0D0D" strokeWidth="1.5" />;
      })}
    </svg>
  );
}
...
function ComicBubble() {
  return (
    <div style={{ animation: "bubblePop 0.35s cubic-bezier(0.16,1,0.3,1) forwards", position: "relative" }}>
      <div style={{ background: "#0D0D0D", borderRadius: 24, padding: 5, boxShadow: "6px 6px 0 #0D0D0D" }}>
        <div style={{ background: "white", borderRadius: 20, padding: "16px 32px", position: "relative" }}>
          <span style={{
            fontSize: "clamp(2rem, 6vw, 3.2rem)",
            fontFamily: "Impact, Arial Black, sans-serif", fontWeight: 900,
            color: "#FF3B30", WebkitTextStroke: "2px #0D0D0D",
...
// ─── GESTRESSTER PAPA (SVG) ───────────────────────────────────────────────────
function StressedComicDad() {
  return (
    <svg width="160" height="250" viewBox="0 0 160 250" fill="none">
      {/* Haar */}
      <path d="M42 45 Q48 18 62 26 Q68 12 80 18 Q92 12 98 26 Q112 18 118 45" fill="#2D1A00" stroke="#0D0D0D" strokeWidth="2"/>
      {/* Kopf */}
      ...
      {/* Körper */}
      <path d="M30 110 Q22 125 24 175 Q26 215 80 218 Q134 215 136 175 Q138 125 130 110 Q116 96 80 96 Q44 96 30 110Z" fill="#FF3B30" stroke="#0D0D0D" strokeWidth="4"/>
      {/* Schild */}
...
// ─── TÜR MIT 3D-SCHATTEN ─────────────────────────────────────────────────────
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function ComicDoor({ deadpoolData }: { deadpoolData: any }) {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center" style={{ background: "#FFD60A" }}>
      ...
      {/* Dunkle Türöffnung */}
      <div style={{ position: "absolute", width: W, height: H, background: "linear-gradient(180deg, #111 0%, #0D0D0D 100%)", borderRadius: borderR, zIndex: 1, boxShadow: "inset 0 0 40px rgba(0,0,0,0.8)" }}/>

      {/* Wand-Schatten rechts */}
      <div style={{
        position: "absolute", left: `calc(50% + ${W / 2 - 6}px)`, top: "50%",
        width: `${shadowWidth}px`, height: H, transform: "translateY(-50%)",
        background: "linear-gradient(to right, rgba(0,0,0,0.45) 0%, rgba(0,0,0,0.12) 60%, transparent 100%)",
        transition: "width 0.65s ease-in-out", zIndex: 6, pointerEvents: "none",
      }}/>

      {/* Tür-Panel */}
      <div style={{ perspective: "1100px", position: "absolute", zIndex: 4 }}>
        <div style={{
          width: W, height: H, background: doorBg, borderRadius: borderR,
          transformOrigin: "left center", transform: `rotateY(${doorAngle}deg)`,
      ...
      {/* Superhero tritt heraus — Lottie Deadpool */}
      {dp === "hero" && deadpoolData && (
        <div style={{ position: "absolute", zIndex: 8, animation: "heroEntry 0.75s cubic-bezier(0.16,1,0.3,1) forwards" }}>
          <div style={{ width: 240, height: 180, filter: "drop-shadow(6px 6px 0 rgba(0,0,0,0.4))" }}>
            <Lottie animationData={deadpoolData} loop={true} autoplay={true} />
          </div>
          {/* PAPA-Schriftzug unter dem Hero */}
          <div style={{ textAlign: "center", marginTop: 8 }}>
            <span style={{ fontFamily: "Impact, Arial Black, sans-serif", fontSize: "1.4rem", color: "#FF3B30", WebkitTextStroke: "2px #0D0D0D", letterSpacing: 4 }}>
              SUPER PAPA
            </span>
          </div>
        </div>
      ...
      {/* Auch wenn kein Lottie: SVG Fallback */}
      {dp === "hero" && !deadpoolData && (
        <div style={{ position: "absolute", zIndex: 8, animation: "heroEntry 0.75s cubic-bezier(0.16,1,0.3,1) forwards", filter: "drop-shadow(4px 4px 0 rgba(0,0,0,0.3))" }}>
          <HeroComicDad />
        </div>
      )}
    </div>
...
function HeroComicDad() {
  return (
    <svg width="180" height="270" viewBox="0 0 180 270" fill="none">
      ...
      <path d="M48 52 Q54 26 66 34 Q72 18 80 24 Q88 18 94 34 Q106 26 112 52" fill="#2D1A00" stroke="#0D0D0D" strokeWidth="2"/>
      ...
      <path d="M30 112 Q22 128 24 178 Q26 218 80 221 Q134 218 136 178 Q138 128 130 112 Q116 98 80 98 Q44 98 30 112Z" fill="#FF3B30" stroke="#0D0D0D" strokeWidth="4"/>
      ...
      <g transform="translate(138, 5)">
        ...
        <path d="M6 22 Q4 6 12 2 L28 2 Q36 6 34 22 Q34 36 20 42 Q6 36 6 22Z" fill="#FFD60A" stroke="#0D0D0D" strokeWidth="3"/>
...
export default function IntroAnimation({ onDone }: { onDone: () => void }) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const phaseTimers = useRef<any[]>([]);

  // Lottie laden + Phasen-Zeitplan starten
  useEffect(() => {
    let mounted = true;

    Promise.all([
      fetch("/animations/deadpool_clean.json").then(r => r.json()),
      fetch("/animations/fireworks.json").then(r => r.json()),
    ]).then(([deadpool, fireworks]) => {
      if (!mounted) return;
      setAnimData({ deadpool, fireworks });
      setPhase("walking");

      // Alle Phasen-Timer auf einmal planen — werden nicht durch Phase-Wechsel gecancelled
      ...
      phaseTimers.current = schedule.map(([p, ms]) =>
        setTimeout(() => { if (mounted) setPhase(p); }, ms)
      );
    });
  ...
  // Objekte nacheinander einblenden
  useEffect(() => {
  ...
  if (phase === "loading") {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "#FFFDE7" }}>
        <div style={{
          fontFamily: "Impact, Arial Black, sans-serif", fontSize: "2rem", color: "#0D0D0D",
          animation: "pulse 0.8s ease-in-out infinite",
          border: "3px solid #0D0D0D", padding: "12px 32px", borderRadius: 8,
          boxShadow: "4px 4px 0 #0D0D0D", background: "white",
        }}>
          ⚡ LOADING...
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden select-none" style={{ background: "#FFFDE7" }}>

      {/* SKIP-Button */}
      <button onClick={skip} style={{
        position: "absolute", top: 20, right: 20, zIndex: 60,
        fontFamily: "Impact, Arial Black, sans-serif", fontSize: "0.85rem", fontWeight: 900,
        border: "2px solid #0D0D0D", borderRadius: 6,
        background: "rgba(255,255,255,0.85)", color: "#0D0D0D",
      ...
      {inChaos && (
        <>
          ...
          {/* Boden */}
          <div style={{ position: "absolute", bottom: "10%", left: 0, right: 0, height: 4, background: "#0D0D0D", opacity: 0.18 }}/>
          ...
          {DROP_OBJECTS.map((o, i) =>
            bangs.includes(i) ? <ComicBang key={`bang-${i}`} text={o.bang} x={o.bx} y={o.by} /> : null
          )}

          {/* Comic Panel Rahmen */}
          <div style={{ position: "absolute", inset: 12, border: "4px solid rgba(0,0,0,0.07)", borderRadius: 8, pointerEvents: "none" }}/>

          {/* HAUPTFIGUR — Gestresster Papa SVG */}
      ...
      {phase === "zoom" && (
        <div className="absolute inset-0 flex items-center justify-center overflow-hidden"
          ...
          <div style={{ position: "relative", textAlign: "center", animation: "zoomPapalapapp 2s cubic-bezier(0.4,0,1,1) forwards" }}>
            ...
            <span style={{
              position: "relative",
              fontFamily: "Impact, Arial Black, sans-serif", fontSize: "clamp(3.5rem, 9vw, 6rem)", fontWeight: 900,
              letterSpacing: "6px", color: "#FF3B30", WebkitTextStroke: "4px #0D0D0D",
              textShadow: "6px 6px 0 #0D0D0D", transform: "rotate(-4deg)", display: "block",
            }}>
              PAPALAPAPP
            </span>
          </div>
      ...
      {/* ══ TÜR ═════════════════════════════════════════════════════════════════ */}
      {phase === "door" && <ComicDoor deadpoolData={animData?.deadpool ?? null} />}

      {/* ══ PARADISE ════════════════════════════════════════════════════════════ */}
      {phase === "paradise" && (
        <div className="absolute inset-0 flex flex-col items-center justify-center" style={{ background: "#FFFDE7", overflow: "hidden" }}>
          ...
          {animData?.fireworks && (
            <div style={{ position: "absolute", inset: 0, opacity: 0.75, pointerEvents: "none" }}>
              <Lottie animationData={animData.fireworks} loop={true} autoplay={true} style={{ width: "100%", height: "100%" }} />
            </div>
          )}

          {/* Großer Starburst */}
          <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: 700, height: 600, pointerEvents: "none", opacity: 0.35 }}>
            <StarBurst color="#FFD60A" size={700} />
          </div>

          {/* Content */}
          <div className="relative text-center px-4" style={{ animation: "paradiseFlyIn 0.9s cubic-bezier(0.16,1,0.3,1) forwards" }}>
            <div style={{
              display: "inline-block", marginBottom: 20,
              background: "#0057FF", border: "3px solid #0D0D0D",
              borderRadius: 999, padding: "6px 28px", boxShadow: "4px 4px 0 #0D0D0D",
            }}>
            ...
            <div>
              <h1 style={{
                fontFamily: "Impact, Arial Black, sans-serif", fontSize: "clamp(4rem, 15vw, 10rem)",
                lineHeight: 0.88, color: "#FF3B30", WebkitTextStroke: "3px #0D0D0D",
                textShadow: "6px 6px 0 #0D0D0D", display: "block",
              }}>PAPA</h1>
              <h1 style={{
                fontFamily: "Impact, Arial Black, sans-serif", fontSize: "clamp(4rem, 15vw, 10rem)",
                lineHeight: 0.88, color: "#FFD60A", WebkitTextStroke: "3px #0D0D0D",
                textShadow: "6px 6px 0 #0D0D0D", display: "block",
              }}>PARADIES</h1>
            </div>
